...
var RTCMediaHandler = function(session, constraints, peer, stream) {
  constraints = constraints || {};

  this.session = session;
  this.localMedia = stream;
  this.peerConnection = stream;
 ...
 * @private
 */
RTCSession.prototype.connect = function(target, options, sdp, peer, stream) {
  options = options || {};
  ...
  this.rtcMediaHandler = new RTCMediaHandler(this, RTCConstraints, peer, stream);
  ...
  if (invalidTarget) {
    ...
    this.sendInitialRequest(mediaConstraints, sdp);
  }
};
 ...
 * @private
 */
RTCSession.prototype.sendInitialRequest = function(constraints, offer) {
    var self = this,
 ...
 *
 */
UA.prototype.call = function(target, options, sdp, peer, stream) {
  var session;

  session = new JsSIP.RTCSession(this);
  session.connect(target, options, sdp, peer, stream);
};
